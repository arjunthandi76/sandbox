...
?>
    ...
    if (count($items>0)) {
      foreach ($items as $delta => $item) {
       ...
       // dpm($item);
        if ($element['#view_mode'] == "teaser") {
          $title = $item['#title'];
          $catlinkname = strtolower ($title);
          $output_str .=  l($title, 'events/posts/' . $catlinkname);
          $contentexists= true;
          
        } else {
          //dpm(strtolower ($item['#items'][0]['data']));
      ...
      if ($element['#view_mode'] == "teaser" && $contentexists) {
        //      $output_str = rtrim($output_str, " | ");
          // $output_str = rtrim($output_str, '<div class="label-inline"> | </div>');
          //$output_str = '<div class="field">'. $output_str . '</div>';
          $output_str = '<div class="field field_event_category"><div class="label-inline"></div>'.$output_str.'</div>';
      } else {
        $output_str = rtrim($output_str, " | ");
        if (!$label_hidden) {
          print '<div class="label-above field_event_category">'. $label . '</div>';
        };   
      }
