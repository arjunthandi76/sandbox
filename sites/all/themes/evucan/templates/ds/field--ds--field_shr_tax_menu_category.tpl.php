    ...
    if (count($items>0)) {
      foreach ($items as $delta => $item) {
       ...
       // dpm($item);
        if ($element['#view_mode'] == "teaser") {
          ...
          $output_str .=  l($title, 'posts/' . $catlinkname);
          $contentexists= true;
          
        } else {
          //dpm(strtolower ($item['#items'][0]['data']));
          $title = $item['#items'][0]['data'];
          $catlinkname = strtolower ($item['#items'][0]['data']);
          $output_str .=  l($title, 'posts/' . $catlinkname) . " | ";
          $contentexists= true;          
        }
      ...
      if ($element['#view_mode'] == "teaser" && $contentexists) {
        //      $output_str = rtrim($output_str, " | ");
          ...
          $output_str = '<div class="field field-name-field-shr-tax-menu-category"><div class="label-inline"></div>'.$output_str.'</div>';
      } else {
        $output_str = rtrim($output_str, " | ");
        if (!$label_hidden) {
