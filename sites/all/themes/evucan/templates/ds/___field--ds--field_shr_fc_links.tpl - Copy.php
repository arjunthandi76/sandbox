...
?>
    ...
    <?php
      ...
      $title = "";
      $image = "";
      $item_key = key($item['entity']['field_collection_item']);
      if (!empty($item['entity']['field_collection_item'][$item_key]['field_link']) && isset($item['entity']['field_collection_item'][$item_key]['field_link'])) {
        $link = $item['entity']['field_collection_item'][$item_key]['field_link']['#items'][0]['url'];
        $title = $item['entity']['field_collection_item'][$item_key]['field_link']['#items'][0]['title'];
      }

      if (!empty($item['entity']['field_collection_item'][$item_key]['field_link']['#object']->field_tax_social_site[LANGUAGE_NONE][0]['taxonomy_term']->field_term_social_icon[LANGUAGE_NONE][0]['uri'])) {
        $image_uri = $item['entity']['field_collection_item'][$item_key]['field_link']['#object']->field_tax_social_site[LANGUAGE_NONE][0]['taxonomy_term']->field_term_social_icon[LANGUAGE_NONE][0]['uri'];        
      ...
      if ($image !== "") {
        ...
        $with_icons_str .= '<a class="with_icon_link" href="'.$link.'"><img alt="'. $title .'" src="'. $base_url . "/".$image .'"></a>';
      } else {
        $without_icons_str .= '<a class="without_icon_link" href="'.$link.'">'. $title .'</a> | ';
      }

    ?>
    <?php endforeach; ?>
    <?php
      $without_icons_str = rtrim($without_icons_str, " | ");
      $title = "<h2>". $label ."</h2>";
      $sub_title = "";
      $closing="";
      if (($with_icons_str !== "") && ($without_icons_str !=="")) {
        $sub_title= '<div id="more-links"><b>More links: </b>';
        $closing = "</div>";
      }
      
      
      print $title . $with_icons_str . "<br/>" . $sub_title . $without_icons_str . $closing;
    ?>  
    <?php } ?>
