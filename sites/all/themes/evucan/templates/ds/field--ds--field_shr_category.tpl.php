...
?>
    ...
    if (count($items>0)) {
      foreach ($items as $delta => $item) {
        dpm($item);
        dpm("field_shr_category");
        $output_str .= "";
        //print_r($item);
        $item_key = key($item['entity']['field_collection_item']);
        $href = $item['entity']['field_collection_item'][$item_key]['field_fc_category'][0]['#href'];
        $title = $item['entity']['field_collection_item'][$item_key]['field_fc_category'][0]['#title'];
