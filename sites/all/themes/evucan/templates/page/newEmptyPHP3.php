...
dpm("-1-");
        ...
        $catlinkname =str_replace (" ", "-", $catlinkname );
        $output_str .=  l($title, 'directory/posts/' . $catlinkname);
        $contentexists= true;
      } else {
dpm("-2-"); 
        $title = $item['#title'];
    ...
    if ($element['#view_mode'] == "teaser" && $contentexists) {
        $output_str = '<div class="field field-dir-main-business-category"><div class="label-inline">'.$label.'</div>'.$output_str.'</div>';
    } else {
      $output_str = rtrim($output_str, " | ");
      if (!$label_hidden) {
        print '<div class="label-above field-dir-main-business-category">'. $label . '</div>';
      };   
    }
