...
<div id="page" class="container <?php print $classes;  ?>">
    ...
    <?php if ($site_logo || $site_name || $site_slogan): ?>
      ...
      <div<?php print $branding_attributes; ?>>
        ...
        <div id="logo-right-side">
          ...
          <?php if ($site_slogan): ?>
          <div<?php print $site_slogan_attributes; ?>><h2><?php print $site_slogan; ?></h2></div>
        </div>
          <?php endif; ?>               
        <?php endif; ?>
  ...
  <!-- Navigation elements -->
  <?php 
  $raw_block = block_load('tb_megamenu', 'main-menu');
        ...
        <<?php print $tag; ?> id="main-content">
          ...
          <?php if ($content = render($page['content'])): ?>
            <div id="content" class="region">
              <?php
                $frontpage = views_get_view('frontpage');
                $frontpage->set_display('block_home_slideshow');
                //last_step_tutorial_view->set_arguments(array(key($query_result['node'])));
                $frontpage->execute();
                print $frontpage->render();  
              ?>
              <div id="home-left" class="front-content-block"> 
              <?php
                $frontpage = views_get_view('frontpage');
                $frontpage->set_display('block_home');
                //last_step_tutorial_view->set_arguments(array(key($query_result['node'])));
                print "<h2 class='block-header block-header-sub block-header-sub1'>Business, Shopping and Food</h2>";
                $frontpage->execute();
                print $frontpage->render();
                //dpm("SHAMONE:");
                //dpm(taxonomy_vocabulary_load(21));
              ?>
              </div>
              <div id="home-right" class="front-content-block"> 
              <?php
                $frontpage = views_get_view('frontpage');
                $frontpage->set_display('block_home_2');
                //last_step_tutorial_view->set_arguments(array(key($query_result['node'])));
                print "<h2 class='block-header block-header-sub block-header-sub2'>Community and Culture</h2>";
                $frontpage->execute();
                print $frontpage->render();  
              ?>
              </div>
